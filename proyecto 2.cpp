/* Nombres: Ferney Hurtado, Juan Diego Ramirez
	Proyecto 3*/
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
struct producto pro[]={//lista productos
	"Tomate",2000,6,0,
...
void pagos()
{	int doo,caja=100000,n=0,cp=0,total1=0,total2=0,devuelta=0,dinero=0;
cout<<endl<<endl;
...
do
{
	...
	total2=total2+total1;//varios productos
	
	if(cp>=(pro[n].cpro*0.5))//si productos adquiridos es mayor o igual al 50% de los productos en stock el contador pasa a 1
	{
		pro[n].c+=1;
	}
...
if(devuelta<0)
{
	devuelta=(devuelta*-1);//convertir a positivo
...
void inventario()
{
	...
	if(c==1234)//contraseña
	{
		...
		for (int i=0;i<50;i++)
		{
			if (pro[i].c>0)
			{
				int s=pro[i].precio*0.8;//precio de adquisicion
...
void preciop()
{
	...
	if(c==1234)//contraseña
	{
		...
		cin>>n;//numero de producto en lista
		cout<<"Ingrese el nuevo precio del producto"<<endl;
		cin>>m;//precio que se va asignar
		pro[n].precio=m;//entregar variable
	}
	else
	{
		system("cls");
...
{	
	...
	while (o>=1&&o<5);//controlador de do while
}
